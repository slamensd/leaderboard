...
if ($batch_id && $_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if($update_status == 'remove'){
		$status_message = $lang['filter-scores-log-batch-remove-success'];
		...
		$status_message = $lang['filter-scores-log-batch-active-success'];
		foreach ($batch_id as &$filter_id) {
			$data_to_store['status'] = 1;
			$db->where('id', $filter_id);
			$status = $db->update ($table_filter_scores, $data_to_store);
		...
		$status_message = $lang['filter-scores-log-batch-inactive-success'];
		foreach ($batch_id as &$filter_id) {
	...
	$_SESSION['info'] = $completeRecord.$lang['filter-scores-log-batch-of'].$totalRecord.$status_message;
	header('location: filter_scores.php');
	exit;
}